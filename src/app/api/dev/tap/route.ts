...
export async function POST(req: NextRequest) {
  ...
  // Demo switch: lets the dashboard's Approve / Reject / "I'm on it" buttons work on the deployed URL.
  const demoOpen = process.env.ALLOW_DEMO_TAP === "1";
  const secretHeader = req.headers.get("x-dev-tap-secret");
  const secretMatches =
    !!process.env.DEV_TAP_SECRET && secretHeader === process.env.DEV_TAP_SECRET;

  if (!isDev && !demoOpen && !secretMatches) {
